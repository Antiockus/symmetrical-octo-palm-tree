...
class Db
{
    ...
    static function connect()
    {
        if (!self::connected()) {
            try {
                self::$db_connection = new \PDO('mysql:host=' . DB_HOST . ';dbname=' . DB_NAME . ';charset=UTF8', DB_USER, DB_PASS);
    ...
    protected function prepareSetup($sql, $values = null)
    {
        self::connect();
    ...
    static function parseList($list)
    {
        ...
        foreach ($parts as $i => $part) {
            $part = str_replace("\n", "'", $part); //convert new lines to commas
            $part = preg_replace('#^\\\\r|([^\\\\])(\\\\\\\\)*(\\\\r)#', "$1$2\r", $part);
            $part = preg_replace('#^\\\\n|([^\\\\])(\\\\\\\\)*(\\\\r)#', "$1$2\n", $part);
